...
pub fn decode_text(text: &str, substitution_table: &SubstitutionTable<&str, char>) -> String {
	...
	let words_decoded: Vec<String> = words.iter().map(|word| {
		word.iter()
			.map(|char| {
				substitution_table.decoding
...
pub fn squish_text(text: &str) -> String {
	text.replace("11", "4").replace("22", "5").replace("33", "6")
}

#[inline]
pub fn unsquish_text(text: &str) -> String {
	text.replace("4", "11").replace("5", "22").replace("6", "33")
}
